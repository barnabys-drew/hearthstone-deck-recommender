...
If the URL only works inside your browser session, save the JSON response to a file instead. For advanced local use, a Cookie header can be supplied via `--collection-cookie-file PATH` ('-' reads stdin, `chmod 600` the file) or the `HS_COLLECTION_COOKIE` / `HS_COLLECTION_COOKIE_FILE` environment variables — all of these stay out of shell history. The raw `--collection-cookie` flag is deprecated. Never put cookies in commits, screenshots, or bug reports.
